...
namespace Nop.Services.Customers
{
    ...
    public partial class CustomerService : ICustomerService
    {
        ...
        public Boolean CheckLogin(int id, string guid) {
            var status=_customerRepository.Table.Where(w => w.Id == id && w.logguid.Equals(guid)).Select(s => s.logstatus).FirstOrDefault();
            if (status == 1)
            {
                return true;
            }
