...
namespace Nop.Plugin.Misc.WebApiServices.Controllers
{
    public class CheckoutController : BaseApiController
    {
        ...
        public IHttpActionResult Index(LoginStatus login)
        {
            if (!_customerService.CheckLogin(login.userid, login.logguid))
                return Ok(new { success = false, msg = "登录超时" });
            _workContext.CurrentCustomer = _customerService.GetCustomerById(login.userid);
            ...
            //reset checkout data
            _customerService.ResetCheckoutData(_workContext.CurrentCustomer, currentStoreId);

            //validation (cart)
            var checkoutAttributesXml = _workContext.CurrentCustomer.GetAttribute<string>(SystemCustomerAttributeNames.CheckoutAttributes, _genericAttributeService, currentStoreId);
        ...
        public IHttpActionResult Completed(int? orderId)
        {
            //validation
            if ((_workContext.CurrentCustomer.IsGuest() && !_orderSettings.AnonymousCheckoutAllowed))
                return Ok();
        ...
        [NonAction]
        protected IHttpActionResult OpcLoadStepAfterShippingMethod(List<ShoppingCartItem> cart)
        {
            ...
            if (isPaymentWorkflowRequired)
            {
                ...
                else
                {
                    ...
                    return Json(new
                    {
                       success=true,goto_section = "payment_method"
                    });
                }
            }
            else
        ...
        [NonAction]
        protected IHttpActionResult OpcLoadStepAfterPaymentMethod(IPaymentMethod paymentMethod, List<ShoppingCartItem> cart)
        {
        ...
        public IHttpActionResult OnePageCheckout()
        {
            //validation
            var cart = _workContext.CurrentCustomer.ShoppingCartItems
                .Where(sci => sci.ShoppingCartType == ShoppingCartType.ShoppingCart)
        ...
        /// <returns></returns>
        [HttpPost]
        public IHttpActionResult OpcBillingForm([FromBody]LoginStatus login)
        {
            if (!_customerService.CheckLogin(login.userid, login.logguid))
                return Ok(new { success = false, msg = "登录超时" });
            _workContext.CurrentCustomer = _customerService.GetCustomerById(login.userid);
        ...
        [HttpPost]
        public IHttpActionResult OpcShippingForm([FromBody]LoginStatus login)
        { 
            if (!_customerService.CheckLogin(login.userid, login.logguid))
                return Ok(new { success = false, msg = "登录超时" });
            _workContext.CurrentCustomer = _customerService.GetCustomerById(login.userid);
            var shippingAddressModel = PrepareShippingAddressModel(prePopulateNewAddressWithCustomerFields: true);
            return Ok(shippingAddressModel);
        }
        [HttpPost]
        public IHttpActionResult ShippingMethodForm([FromBody]LoginStatus login) {
          
            if (!_customerService.CheckLogin(login.userid, login.logguid))
                return Ok(new { success = false, msg = "登录超时" });
           var cart = _customerService.GetCustomerById(login.userid).ShoppingCartItems
                    .Where(sci => sci.ShoppingCartType == ShoppingCartType.ShoppingCart)
                    .LimitPerStore(_storeContext.CurrentStore.Id)
                    .ToList();;
        ...
        [HttpPost]
        public IHttpActionResult PaymentMethodForm([FromBody]LoginStatus login)
        {
            if (!_customerService.CheckLogin(login.userid, login.logguid))
                return Ok(new { success = false, msg = "登录超时" });
            var cart = _customerService.GetCustomerById(login.userid).ShoppingCartItems
       ...
       [HttpPost]
        public IHttpActionResult OpcSaveBilling(System.Net.Http.Formatting.FormDataCollection formdata)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (form.ContainsKey("Email"))
                        newaddress.Email = form["Email"];
                    if (form.ContainsKey("FirstName"))
                        newaddress.FirstName = form["FirstName"];
                    if (form.ContainsKey("LastName"))
                        newaddress.LastName = form["LastName"];
                    if (form.ContainsKey("Company"))
                        newaddress.Company = form["Company"];
                    if (form.ContainsKey("ZipPostalCode"))
                        newaddress.ZipPostalCode = form["ZipPostalCode"];
                    if (form.ContainsKey("City"))
                        newaddress.City = form["City"];
                    if (form.ContainsKey("CountryId"))
                        newaddress.CountryId = Convert.ToInt32(form["CountryId"]);
                    if (form.ContainsKey("StateProvinceId"))
                        newaddress.StateProvinceId = Convert.ToInt32(form["StateProvinceId"]);
                    if (form.ContainsKey("Address"))
                        newaddress.Address1 = form["Address1"];
                    if (form.ContainsKey("Address2"))
                        newaddress.Address2 = form["Address2"];
                ...
                if (cart.RequiresShipping())
                {
                    ...
                    var shippingAddressModel = PrepareShippingAddressModel(prePopulateNewAddressWithCustomerFields: true);
                    return Ok(new { success = true, gosection = "shipping", shippingAddress = shippingAddressModel });
                }
                else
                {
       ...
       [HttpPost]
       public IHttpActionResult OpcSaveShipping(System.Net.Http.Formatting.FormDataCollection formdata)
       {
           ...
           try
           {
              ...
              /*    TryUpdateModel(pickUpInStoreModel);
               ...
               {
                  // return Ok(new { success = true, gosection = "shippingMethod",shippingMethodModel = shippingMethodModel });
      ...
      [HttpPost]
       public IHttpActionResult OpcSaveShippingMethod(System.Net.Http.Formatting.FormDataCollection formdata)
       {
           IDictionary<string, string> form = new Dictionary<string, string>();
           foreach (var t in formdata)
           {
      ...
      public IHttpActionResult OpcSavePaymentMethod(System.Net.Http.Formatting.FormDataCollection formdata)
      {
          IDictionary<string, string> form = new Dictionary<string, string>();
          foreach (var t in formdata)
          {
          ...
          try
          {
              ...
              if (!isPaymentWorkflowRequired)
              {
                  ...
                  return Json(new
                  {
                          success=true,confirmOrderModel=confirmOrderModel,goto_section = "confirm_order"
                  });
              }
      ...
      public IHttpActionResult OpcSavePaymentInfo(System.Net.Http.Formatting.FormDataCollection formdata)
      {
          IDictionary<string, string> form = new Dictionary<string, string>();
          foreach (var t in formdata)
          {
          ...
          try
          {
              ...
              var paymentController = System.Web.Mvc.DependencyResolver.Current.GetService(paymentControllerType) as BasePaymentController;
             ...
             */
              if (ModelState.IsValid)
              {
               ...
               //   _httpContext.Session["OrderPaymentInfo"] = paymentInfo;
                  ...
                  return Json(new
                  {
                    success=true,goto_section = "confirm_order"
                  });
              }
      ...
      [HttpPost]
      public IHttpActionResult OpcConfirmOrder([FromBody]LoginStatus login)
      {
          if (!_customerService.CheckLogin(login.userid, login.logguid))
              return Ok(new { success = false, msg = "登录超时" });
          _workContext.CurrentCustomer = _customerService.GetCustomerById(login.userid);
          try
          {
              ...
              else
              {
                  ...
                  return Json(new { success = true,error=confirmOrderModel.Warnings});
              }
          }
          catch (Exception exc)
          {
