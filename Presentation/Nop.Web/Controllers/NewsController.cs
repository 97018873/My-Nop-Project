...
namespace Nop.Web.Controllers
{
    ...
    public partial class NewsController : BasePublicController
    {
        ...
        public ActionResult HomePageNewss() {
            if (!_newsSettings.Enabled || !_newsSettings.ShowNewsOnMainPage)
                return Content("");
            /*
             var cacheKey = string.Format(ModelCacheEventConsumer.HOMEPAGE_NEWSMODEL_KEY, _workContext.WorkingLanguage.Id, _storeContext.CurrentStore.Id);
        ...
        public ActionResult List(NewsPagingFilteringModel command,int category=0)
        {
...
/*
            ...
            if (category != 0)
            {
                var query = newsItems.Where(w => w.Category == category).AsQueryable();
                newsItem = newsItems.Where(w => w.Category == category).ToList();
                newsItems =new PagedList<NewsItem>(query, command.PageNumber - 1, command.PageSize);
            }
            else {
 ...
 */
                ...
                var newsItems = _newsService.GetAllNews(_workContext.WorkingLanguage.Id, _storeContext.CurrentStore.Id,
                         command.PageNumber - 1, command.PageSize);
            if(category!=0){
                newsItems = _newsService.GetNewsByCategory(command.PageNumber - 1, command.PageSize, category);
            }
            newsItem = newsItems.ToList();
        ...
        /// <returns></returns>
        public ActionResult ListByCategory(NewsPagingFilteringModel command, int category, int count,bool flag=false)
        {
            var model = new NewsItemListModel();
            var newsItems = _newsService.GetNewsByCategory(
                0,count,category);

            var list = newsItems.ToList<NewsItem>();
            model.NewsItems = list
               ...
               .ToList();
            ViewData["DOMID"] = _newsCategoryService.GetById(category).DomId;
            ViewData["FLAG"] = flag;
            return PartialView(model);
        }
